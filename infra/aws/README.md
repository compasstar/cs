...
3. Web(S3) (`https://domain.com`)
   - 도메인 (Route53)
   - HTTPS (AWS Certificate Manager) 
4. 로드밸런서(ELB)(`https://api.domain.com`)
   - 도메인 Route53
   - HTTPS (AWS Certificate Manager) 
5. Server(EC2) 
   - 파일 및 이미지 (S3)
   - 만약 ELB 없다면 도메인, HTTPS 적용
6. DB(RDS)
...
- 네이밍 규칙: 프로젝트-역할-환경-번호 (Sandbox-Backend-01 MYP-Frontend-Prod-01, KPS-Admin-Test-02)
- 키 페어 생성: RSA 생성, 확장자 .pem 선택 (sandbox-key)
- OS: Ubuntu
- 스토리지: 30GiB gp3 (프리티어 기준)
- EC2 접속: 인스턴스에 연결 > EC2 Instance Conneect을 사용하여 연걸

### 보안그룹
- 인바운드: SSH, TCP, PORT 22, [내 아이피]/32 (여기서 32는 내 아이피 주소 하나만을 뜻함, 0은 모든 IP주소 범위)
- 인바운드: 사용자지정TCP, TCP, PORT 8080, Frontend 주소 (프론트엔드 서버만이 백엔드 API에 접속하도록 허용)
- 아웃바운드: 모든트래픽, 모든 프로토콜, 모든 포트, 0.0.0.0/0 (외부 인터넷으로 나가는 모든 트래픽 허용)
